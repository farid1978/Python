----- [01_14_usercell_fileaccess.sql]
----- [2/3/4/5G]

insert into table sdr_ps.TBL_PREFIX_cee_user_cell_fa_tbl
select 
  ...
  , case when prot_category = 5 then 'Game' else 'File_Access' end as srv
  ...
  -- fa experience
  -- exp_srv_filetrans_delay = file sharing response total delay / file sharing  response total times
  , sum(case when service_valid_flag = 1 and record_type in (10,11,15) and file_req_rsp_delay > 0 then file_req_rsp_delay else 0 end) as file_sharing_response_total_delay
  , sum(case when service_valid_flag = 1 and record_type in (10,11,15) and file_req_rsp_delay > 0 then 1 else 0 end) as file_sharing_response_total_times
  -- exp_srv_filetrans_dl_thrp = file sharing valid total download traffic * 8 / 1024 / (file sharing valid total download duration)
  , sum(case when service_valid_flag = 1 and record_type in (10,11,15) and (l4_dw_throughput/1024 >= 500) then l7_dw_goodput_full_mss else 0 end) as file_sharing_dl_goodput
  , sum(case when service_valid_flag = 1 and record_type in (10,11,15) and (l4_dw_throughput/1024 >= 500) then datatrans_dw_duration/1000000000 else 0 end) as file_sharing_dl_duration
  -- game RTT
