----- [01_15_usercell_im.sql]
----- [2/3/4/5G]

insert into table sdr_ps.TBL_PREFIX_cee_user_cell_im_tbl
select 
  ...
  , case when prot_category = 5 then 'Game' else 'IM' end as srv
  ...
  -- IM experience
  , sum(case when (service_valid_flag = 1 and interactive_num > 0) then interactive_num else 0 end) as interactive_num
  , sum(case when (service_valid_flag = 1 and interactive_num > 0) then interactive_delay else 0 end) as interactive_total_delay
